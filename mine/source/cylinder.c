...
void	get_cylinder_surface(t_scene *s, t_ray *r, int i)
{
	...
	if (is_vec3_void(r->origin) != 0)
		tmp->oc = vec3_subtraction(r->origin, tmp->point);//세계 좌표로 카메라 중심에서 도형 위치 빼서 벡터 oc 확보
	else
		tmp->oc = vec3_subtraction(s->camera[s->i_cam]->pos,
		tmp->point);//r->origin이 0,0,0이면 파싱한 카메라 중심에서 도형 위치 빼서 벡터 oc 확보
	sub = vec3_dot_product(r->global, tmp->n);//r->global은 캔버스 좌표 P를 세계 좌표 P'로 바꾼 것을 정규화한 것
	tmp->a = 1 - sub * sub;
...
int		is_ray_intersect_cylinder(t_scene *s, t_ray *r, int i)
{
	...
	if (tmp->delta < 0)//delta > 0은 교차점 2개, delta = 0은 1개, delta < 0은 없음
		return (0);
	t = 0;
	tmp->x1 = (sqrt(tmp->delta) - tmp->b) / (2 * tmp->a);//교차점 거리 2개 확보해옴
...
void	draw_cylinder_on_canvas(t_scene s, t_ray *r, int i)
{
	...
	s_now_obj = s.cylinder[i]->filter == 'r' ? 'c' : 0;
	t = r->t;
	tmp = s.cylinder[i];
