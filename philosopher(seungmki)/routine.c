...
/*
	...
	pthread_mutex_lock(&info->fork[philo->r_fork]);//철학자가 1명뿐이면 포크 색인이 [0]뿐이라
	print_state(philo, FORK);
	pthread_mutex_lock(&info->fork[philo->l_fork]);//여기서 걸려버림
	print_state(philo, FORK);
...
{//이 함수에서 잡아야 dead_or_alive()에서 죽음을 기록해도 다른 상태 출력되는 걸 막을 수 있다
	...
	now = get_ms_time() - philo->info->start_time;
	info = philo->info;
	if (state != DIE && now - philo->last_eat > info->time_to_die)
	{//모니터링이 아직 죽음 감지를 못했지만 기아라면 이 함수와 do_routine() 탈출
		pthread_mutex_unlock(&(philo->info->print_state));
		return (0);
	}
	printf("\e[33m%lums \e[96m%d \e[39m", now, philo->name);
	if (state == FORK)
		printf("\e[32mhas taken a fork\e[39m\n");
	else if (state == EAT)
		printf("\e[33mis eating\e[39m\n");
	else if (info->all_alive == FALSE && state == SLEEP)
		printf("\e[96mis sleeping\e[39m\n");
	else if (info->all_alive == FALSE && state == THINK)
		printf("\e[0;35mis thinking\e[39m\n");
	else if (state == DIE)
		printf("\033[0;31mdied\e[39m\n");
	pthread_mutex_unlock(&(philo->info->print_state));
}//죽은 철학자가 있거나 현재 스레드 철학자가 기아면 탈출
*/