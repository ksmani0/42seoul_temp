...
int	get_next_line(int fd, char **line)
{
	...
	{
		rd_buf[rbytes] = 0;
		if ((paste_malloc(&store[fd], rd_buf, line)) != 1)
			return (-1);
		if ((w_num = find_nline(store[fd])) >= 0)
			return (line_str_nline(line, &store[fd], w_num));
	}
	if (rbytes == 0 && store[fd] != 0 && (w_num = find_nline(store[fd])) >= 0)
		return (line_str_nline(line, &store[fd], w_num));
	else if (rbytes == 0 && store[fd] != 0)
		return (line_just_str(line, &store[fd]));
	return (free_store(rbytes, &store[fd], line));
}
